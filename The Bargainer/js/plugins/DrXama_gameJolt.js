...
//==================================================================================================
/*:
 * @plugindesc v1.3.2 - Integração do Game Jolt
 *
 * @author Dr.Xamã
 * 
 * @param Game ID
...
(function () {
        ...
        HTTPS.userAuth(username, userToken, {
            success: (res) => {
                if (res.response.success === 'false') {
                    callback(false);
                    return console.error(res.response.message);
                }
    ...
    Game_Temp.prototype.gameJoltAddUser = function (username, userToken) {
        let user = api.users.filter(user => {
            if (user.username() === String(username)) {
                if (user.userToken() != String(userToken))
                    user._userToken = String(userToken);
                return true;
            }
        });
        if (user.length <= 0) {
            let gamejoltuser = new Game_Jolt_User(String(username), String(userToken));
            api.users.push(gamejoltuser);
            return true;
    ...
    Game_Temp.prototype.gamejoltGetSpriteAvatar = function (username, hue) {
        let url = '',
            user = api.users.filter(user => {
                if (user.username() === String(username)) {
                    url = user.avatarUrl();
                    return true;
                }
        ...
        if (user.length > 0) {
            return ImageManager.gameJoltloadFile(url, hue);
        }
        return ImageManager.gameJoltloadFile('https://i.imgur.com/LLaWfqM.png', hue);
    };

    Game_Temp.prototype.gamejoltGetTypeUser = function (username) {
        let type = '',
            user = api.users.filter(user => {
                if (user.username() === String(username)) {
                    type = user.type();
