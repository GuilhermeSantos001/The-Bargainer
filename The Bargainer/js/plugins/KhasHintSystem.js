...
Game_Event.prototype.khs_update = Game_Event.prototype.update;
let gameEventDelay = { frame1: 0, frame2: 5 };
    ...
    if (list) {
      for (var i = 0; i < list.length; i++) {
        if (list[i] && list[i].code == 108) {
          ...
          if (hint) {
            if (this._hintTitle) {
              this._hintMessage += Window_Base.prototype.convertEscapeCharacters((list[i].parameters[0] + " "));
            } else {
              this._hintTitle = Window_Base.prototype.convertEscapeCharacters(list[i].parameters[0]);
            };
          };
        };
      };
...
Game_Event.prototype.update = function () {
  this.khs_update();
  if (gameEventDelay.frame1 < gameEventDelay.frame2) {
    gameEventDelay.frame1 += .60;
  } else {
    gameEventDelay.frame1 = 0;
